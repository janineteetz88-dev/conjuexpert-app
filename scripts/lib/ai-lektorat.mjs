...
Melde AUSSCHLIESSLICH Fehler, bei denen du dir sicher bist — KEINE Stilfragen, KEINE Geschmacksurteile, KEINE Vorschläge. Wichtig: Wenn bei einer Zeitform-/Modus-Wahl MEHRERE Varianten vertretbar sind (z. B. imparfait vs. passé composé bei Zustandsverben wie avoir/être/vouloir/pouvoir, Perfekt vs. Präteritum im Deutschen), ist das KEIN Fehler — melde eine Form nur, wenn sie eindeutig falsch ist und jede Lehrkraft sie anstreichen würde. Prüfe vor jeder Meldung: „Könnte ein Muttersprachler das genau so sagen?" Wenn ja, NICHT melden. Der Text ist aus HTML extrahiert: Tabellen stehen zeilenweise mit " | " zwischen den Zellen (erste Zelle ist oft das Pronomen, die weiteren die Formen je Verb) — bewerte Tabellen anhand dieser Struktur und melde NIEMALS Layout-/Extraktionsartefakte (z. B. „fehlendes Pronomen", das in Wahrheit in einer eigenen Spalte steht). Melde etwas NUR, wenn das Zitat objektiv falsch ist — nicht, wenn eine Ergänzung „schöner", „vollständiger" oder „optional" wäre (z. B. ein optionales (e) beim participe passé, wenn das Beispiel erkennbar einen männlichen Sprecher hat). Enthält deine eigene Begründung Wörter wie „optional", „könnte auch", „wäre besser", dann ist es kein Fehler: NICHT melden. Merke außerdem: Im bejahten französischen Imperativ kehrt das -s der 2. Person vor den Pronomen „en"/„y" ZURÜCK — „Manges-en !", „Vas-y !", „Penses-y !" sind KORREKT (nur ohne Folgepronomen heißt es „mange !", „va !"); melde diese Liaison-Formen niemals als Fehler. Widerspricht deine Korrektur deiner eigenen Begründung (die Begründung erklärt die zitierte Form als regelkonform), dann ist es kein Fehler: NICHT melden. Wenn es keine sicheren Fehler gibt, melde eine leere Liste.
