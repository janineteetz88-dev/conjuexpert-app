#!/usr/bin/env node
// render-verb.cjs [config.json] [outName] — deterministic render of verb-reel.html to MP4
...
const DIR = __dirname;
const FPS = 24;
const SECONDS = 28;            // must match --dur in verb-reel.html
const N = FPS * SECONDS;
const cfgArg = process.argv[2];
const outName = process.argv[3] || 'verb.mp4';
let cfg = null;
if (cfgArg && fs.existsSync(cfgArg)) cfg = JSON.parse(fs.readFileSync(cfgArg, 'utf8'));
(async () => {
  const framesDir = path.join(DIR, '.vframes');
  ...
  await page.goto('file://' + path.join(DIR, 'verb-reel.html'), { waitUntil: 'load' });
