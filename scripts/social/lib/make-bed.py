...
(reine Mathematik, keine Samples). Styles: summer (Summer-House / Chill-Out,
blumig-atmosphaerisch, Default), calm (Lo-Fi), bright (Pop), pulse (minimal).
...
# ---- SUMMER HOUSE / CHILL-OUT (blumig: lush pads + shimmer arp, no cheesy tune) ----
if STYLE == 'summer':
    BPM = 118.0
    beat = 60.0 / BPM
    bar = 4 * beat
    # 8-bar journey resolving to C: Fmaj7 G Em7 Am7 | Dm7 G Cmaj7 Cmaj7
    CH = [
    ...
    pad=[0.0]*N; bass=[0.0]*N; perc=[0.0]*N; air=[0.0]*N
    nb = int(DUR / bar) + 1
    def chidx(t): return int(t // bar) % len(CH)

    # lush pad: main voices + a high shimmer layer that slowly blooms each bar
    for b in range(nb):
        chord,_ = CH[b % len(CH)]
        s=int(b*bar*SR); e=min(N,int((b+1)*bar*SR)); seg=e-s
        atk=int(0.24*SR); rel=int(0.4*SR); g = 0.22 if b == 0 else 0.30
        batk=int(0.5*SR)                              # slow "bloom" attack for shimmer
        for i in range(seg):
            t=i/SR; ev=env_ar(i,seg,atk,rel)*g; acc=0.0
            for f in chord:
                ph=2*math.pi*f*t; acc+=math.sin(ph)+0.18*math.sin(2*ph)
            pad[s+i]+=acc/(len(chord)*1.18)*ev
            # high airy bloom (octave up, soft, swells in)
            be = (i/batk if i < batk else env_ar(i,seg,batk,rel))*0.09
            hi=0.0
            for f in chord: hi+=math.sin(2*math.pi*f*2*t)
            air[s+i]+=hi/len(chord)*be

    # bassline (off-beat bounce, fat + sub) — kept as-is (you liked it)
    ...
    # blumig shimmer arp: gentle high chord-tone cascade over first 2 beats of each bar,
    # mellow bell tone, long release — pretty & atmospheric, NOT a jaunty melody
    for b in range(nb):
        if b < 1: continue
        chord,_ = CH[b % len(CH)]
        for j,f in enumerate(chord):
            t0=(b*4 + j*0.5)*beat; s0=int(t0*SR); ln=int(0.6*SR)
            note=f*2
            for i in range(ln):
                if s0+i>=N: break
                e=math.exp(-i/(0.28*SR))                # long, soft decay
                atk=int(0.008*SR); a=(i/atk if i<atk else 1.0)
                air[s0+i]+=0.11*a*e*(math.sin(2*math.pi*note*(i/SR))+0.12*math.sin(2*math.pi*note*2*(i/SR)))

    # soft house drums: kick (from bar1) + light clap 2&4 + gentle off-beat hats
    kb=0
    while kb*beat < DUR:
        ...
        if t0 >= bar:
            for i in range(int(0.18*SR)):
                if k0+i>=N: break
                e=math.exp(-i/(0.05*SR)); f=116*math.exp(-i/(0.022*SR))+50
                perc[k0+i]+=0.8*e*math.sin(2*math.pi*f*(i/SR))
            if kb%4 in (1,3):
                prev=0.0
                for i in range(int(0.10*SR)):
                    if k0+i>=N: break
                    e=math.exp(-i/(0.032*SR)); n=random.uniform(-1,1); prev=n-0.3*prev
                    perc[k0+i]+=0.11*e*prev
        h0=int((t0+beat/2)*SR); prev=0.0
        for i in range(int(0.05*SR)):
            if h0+i>=N: break
            e=math.exp(-i/(0.015*SR)); n=random.uniform(-1,1); prev=n-0.6*prev
            perc[h0+i]+=0.09*e*prev
        kb+=1

    # sidechain pump on pad+bass+air
    duck=[1.0]*N; tk=bar
    while tk < DUR:
        k0=int(tk*SR)
        for i in range(int(beat*SR)):
            if k0+i>=N: break
            duck[k0+i]=min(duck[k0+i],0.62+0.38*(i/(beat*SR)))
        tk+=beat
    buf=[0.0]*N
    for i in range(N):
        buf[i]=(pad[i]+bass[i]+air[i])*duck[i]+perc[i]
...
# time-varying low-pass (opens up over first ~3s) + fades + normalize
build = int(3.0 * SR)
y = 0.0
for i in range(N):
    a = LPA
